...
flowScheduler.add(welocome_screenRoutineBegin());
flowScheduler.add(welocome_screenRoutineEachFrame());
flowScheduler.add(welocome_screenRoutineEnd());
...
async function experimentInit() {
  // Initialize components for Routine "welocome_screen"
  welocome_screenClock = new util.Clock();
  ...
  break_text = new visual.TextStim({
    win: psychoJS.window,
    name: 'break_text',
    text: 'لطفا 10 دقیقه استراحت کنید.\n\n\nبا فشردن کلید\n space\nبه صفحه بعد منتقل می شوید.',
    font: 'Arial',
    units: undefined, 
    pos: [0, 0], draggable: false, height: 0.05,  wrapWidth: undefined, ori: 0.0,
    languageStyle: 'Arabic',
...
var frameN;
var continueRoutine;
var welocome_screenMaxDurationReached;
var _key_welcome_allKeys;
var welocome_screenMaxDuration;
var welocome_screenComponents;
function welocome_screenRoutineBegin(snapshot) {
  return async function () {
    TrialHandler.fromSnapshot(snapshot); // ensure that .thisN vals are up to date
    
    //--- Prepare to start Routine 'welocome_screen' ---
    t = 0;
    frameN = -1;
    continueRoutine = true; // until we're told otherwise
    welocome_screenClock.reset();
    routineTimer.reset();
    welocome_screenMaxDurationReached = false;
    // update component parameters for each repeat
    key_welcome.keys = undefined;
    key_welcome.rt = undefined;
    _key_welcome_allKeys = [];
    psychoJS.experiment.addData('welocome_screen.started', globalClock.getTime());
    welocome_screenMaxDuration = null
    // keep track of which components have finished
    welocome_screenComponents = [];
    welocome_screenComponents.push(welcome_text);
    welocome_screenComponents.push(key_welcome);
    
    welocome_screenComponents.forEach( function(thisComponent) {
...
function welocome_screenRoutineEachFrame() {
  return async function () {
    //--- Loop for each frame of Routine 'welocome_screen' ---
    // get current time
    t = welocome_screenClock.getTime();
...
function welocome_screenRoutineEnd(snapshot) {
  return async function () {
    //--- Ending Routine 'welocome_screen' ---
    welocome_screenComponents.forEach( function(thisComponent) {
      if (typeof thisComponent.setAutoDraw === 'function') {
        thisComponent.setAutoDraw(false);
      }
    });
    psychoJS.experiment.addData('welocome_screen.stopped', globalClock.getTime());
    ...
    key_welcome.stop();
    // the Routine "welocome_screen" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset();
...
function trialsLoopBegin(trialsLoopScheduler, snapshot) {
  return async function() {
    ...
    trials.forEach(function() {
      ...
      trialsLoopScheduler.add(feedbacktestRoutineBegin(snapshot));
      trialsLoopScheduler.add(feedbacktestRoutineEachFrame());
      trialsLoopScheduler.add(feedbacktestRoutineEnd(snapshot));
...
function study_phaseRoutineBegin(snapshot) {
  return async function () {
    ...
    study_phaseComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function fixctionRoutineBegin(snapshot) {
  return async function () {
    ...
    fixctionComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function break_phaseRoutineBegin(snapshot) {
  return async function () {
    ...
    break_phaseComponents.forEach( function(thisComponent) {
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
       });
    return Scheduler.Event.NEXT;
...
function end_screenRoutineEachFrame() {
  return async function () {
    ...
    // *key_end* updates
    if (t >= 2 && key_end.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
